...
function checkBrowserSupport() {
    ...
    if (!supports.fetch) {
        ...
        // Показать альтернативную форму
    }
}

// Ленивая загрузка для GitHub Pages
function lazyLoadForGitHub() {
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('TalkaLab - сайт загружен и инициализирован для GitHub Pages');

    // 1. SEO оптимизации для GitHub
    updateCurrentYear();
    checkBrowserSupport();
    lazyLoadForGitHub();
    ...
    // 3. ФИКСИРОВАННЫЙ ХЕДЕР ПРИ СКРОЛЛЕ
    ...
    if (consultationForm) {
        consultationForm.addEventListener('submit', async function(e) {
            ...
            // Настройки бота Telegram для GitHub Pages
            const botToken = '8580940089:AAHEYpW_nPjmnOU6mA47ITlV_wWFOHKIazo';
            const chatId = '-5113146363';
            
            // Формируем сообщение
            const message = `📋 *Новая заявка с GitHub Pages*\n\n` +
                           `👤 *Имя:* ${name}\n` +
                           `📱 *Telegram:* ${telegram}\n\n` +
                           `🕐 *Время:* ${new Date().toLocaleString('ru-RU')}\n` +
                           `🌐 *Источник:* GitHub Pages`;
    ...
    // 9. ЗАКРЫТИЕ МОДАЛЬНЫХ ОКОН ПРИ КЛИКЕ НА ФОН
    ...
    // 10. ОТКРЫВАЕМ ПЕРВЫЙ FAQ ПРИ ЗАГРУЗКЕ
    setTimeout(() => {
    ...
    // 11. ОПТИМИЗАЦИЯ ДЛЯ GITHUB PAGES
    // Добавляем базовый URL для корректной работы ссылок
    if (window.location.hostname.includes('github.io')) {
        const baseUrl = window.location.origin + window.location.pathname.replace(/\/[^\/]*$/, '/');
        console.log('GitHub Pages base URL:', baseUrl);
        
        // Исправляем ссылки для GitHub Pages
    ...
    console.log('✅ Все функции готовы к работе на GitHub Pages');
});

// === ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ GITHUB ===

// Отслеживание ошибок для GitHub Pages
window.addEventListener('error', function(e) {
    console.error('Ошибка на GitHub Pages:', e.error);
    
    // Показываем пользователю дружеское сообщение
    if (e.error && e.error.message && e.error.message.includes('CORS')) {
        console.warn('CORS ошибка на GitHub Pages - это нормально для статических файлов');
    }
});
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', function() {
        navigator.serviceWorker.register('./sw.js').then(function(registration) {
            console.log('ServiceWorker регистрация успешна с областью действия: ', registration.scope);
        }, function(err) {
...
window.addEventListener('load', function() {
    console.log('✅ Страница полностью загружена на GitHub Pages');
    
    // Проверяем все внешние ресурсы
    ...
    externalResources.forEach(resource => {
        ...
        link.rel = 'preconnect';
        link.href = new URL(resource).origin;
        document.head.appendChild(link);
    });
});

// Специальная функция для GitHub Pages CORS
function safeFetch(url, options = {}) {
    return fetch(url, {
        ...
        console.warn('Fetch ошибка (возможно CORS):', err);
...
console.log('TalkaLab функции экспортированы в window.TalkaLab');